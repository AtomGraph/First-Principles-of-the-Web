...
The scope of this result, before anything is built on it. These laws govern composition's *mechanics*: that any two states merge, without permission, in any order, at no cost. They do not promise that merged facts *join* — that two parties' names for one turbine ever meet in a query. No data model can promise that: parties who never communicated have agreed on nothing, in every model ever proposed, and a requirement pretending otherwise would be a coordinator in disguise. What a model does control is the terms — what an unjoined union is worth, and what joining costs once demand prices it worth paying — and Chapter 15 meets that question head-on, where federation stops being algebra and becomes deployment. Here it is enough to be exact about what R2 buys: merge, not meaning across sources; and it buys that completely.
...
That is the bill for anonymity, and it falls exactly on the party that chose anonymity: name your entities and state composes by set arithmetic; leave them unnamed and composition becomes theorem-proving in miniature, payable precisely at the seam where Chapter 6 put `canon`. The web's grain shows through the formalism: the URI is what makes merging cheap. The deployed stack's own list idiom itemizes the bill: an ordered collection encoded as a chain of unnamed cells pays the anonymity price at every link, and the derivation's grain — order booked as a rank fact per member, Chapter 3's lead story — wears better than the chain that mimics a data structure. Order is the recurring case: state it as facts and it merges like facts; fold it into shape and it is taxed like shape.
...
**The one-timeline economy.** Independent evolution (Prop. 4.5) prices the fusion. One component, one timeline: any change — data, layout, theme, query — is a change to the whole term, and the term is the unit of delivery, so the term is the unit of invalidation. The consequences, each one a property from Fielding's own list:
...
One closing observation: the convergence recovers S1 and S2 but stalls before R3 and S4 — the two properties that make it *the web* rather than an app platform that happens to use browsers. The stall has a reason: the last mile's benefits accrue to everyone except the vendor walking it. The properties whose payoffs are private return first; the properties whose payoffs are public wait.
