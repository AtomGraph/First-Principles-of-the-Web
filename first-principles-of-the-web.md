...
S2 asks it of the other two factors as well, and neither got a language. No transformation was standardized for the model, and no presentation. Chapter 10 scored JSON `✗` on evidence of the same shape, because JSON leaves interpretation to applications too. The property graph does not leave it there, so it keeps a tilde where JSON lost one. S4 fails outright: a query goes to an endpoint and the result comes back as a payload with no address of its own, so no intermediate value can be linked or cached. That is GraphQL's cell again, for GraphQL's reason.
